...
- **`osxphotos` ausente após o primeiro `pip install -e ".[dev]" --quiet`:** o pacote está declarado em `pyproject.toml` mas não foi instalado na primeira passada (`--quiet` mascarou o motivo), derrubando `tests/test_apple_photos.py::test_video_entra_junto_com_a_foto` com `ModuleNotFoundError`. `pip install osxphotos` isolado resolveu (arrastou toda a árvore de dependências do pyobjc/osxphotos, ~80 pacotes) — depois disso a suíte completa (844 testes) passou. Não relacionado a este plano; ambiente do worktree, não código.
