...
- `06-05-PLAN.md` (ExifWriteExecutor: dry-run autoritativo, escrita verificada, falha parcial e backup) segue como próximo passo. `formatos.suportado()` já reflete o resultado atualizado e pode ser consumido sem alteração para decidir QUAL formato tentar. **Mas o gate de verificação da escrita real precisa fazer mais que chamar `verificacao.diferenca()` sozinho** — `reclassificar_deslocamentos_de_offset()` é uma função separada, deliberadamente não embutida em `diferenca()`, e nada em produção a chama ainda (só `scripts/testar_escrita_exif.py`). Se 06-05 espelhar o padrão óbvio de `diferenca()` isolado (como o código de medição fazia antes desta correção), toda escrita real em `.jpg`/`.cr2` vai reprovar de novo por `IFD1:ThumbnailOffset`/similares — uma regressão silenciosa da allowlist que acabou de aprovar esses formatos. O executor de 06-05 TEM que chamar `reclassificar_deslocamentos_de_offset(diff, antes, depois, arquivo_antes, arquivo_depois)` depois de `diferenca()`, usando o mesmo padrão de `testar_amostra()`: o backup `<alvo>_original` que `ExifToolWriter.escrever()` sempre deixa (nunca usa `-overwrite_original`) é o par "antes" byte a byte, sem precisar de snapshot extra.
