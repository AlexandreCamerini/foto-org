...
| **Estimated runtime** | **Medido em 2026-08-17** nesta worktree: `scripts/verificar.sh --rapido` = **86.2s** (pytest 844 testes em 80.0s + benchmark de agrupamento 19/19; pula vitest **e** `npm run build`, não só o build). Subset por task (`pytest tests/test_database.py tests/test_repository.py -q`) = **2.7s**. Suíte completa `scripts/verificar.sh` ≈ 90s+. |
...
**Note (medição de 2026-08-17):** `--rapido` a 86.2s cabe no orçamento de 90s, mas com margem de ~4s — por isso ele é o portão de **fim de wave** (§ Sampling Rate), e a verificação por task usa o subset de pytest dos arquivos afetados (2.7s). Isso mantém a latência de feedback por task uma ordem de grandeza abaixo do teto.
...
- **Max feedback latency:** 90 seconds — respeitado com folga por task (subset de pytest, ~3s); o portão de wave `scripts/verificar.sh --rapido` fica em 86.2s medidos.
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 05-01-01 | 05-01 | 1 | LANC-02 | T-05-01/02 | `EXPLAIN QUERY PLAN` da consulta real de `_sob_a_pasta` mostra SEARCH; busca `.ilike` segue insensível a caixa | integration (DB-level) | `.venv/bin/python -m pytest tests/test_indices.py -q --no-header; test $? -eq 1` (RED esperado: exit 1 = falha de asserção, não erro de coleta) | ❌ Wave 0 — criado nesta task | ⬜ pending |
...
| 05-01-03 | 05-01 | 1 | LANC-02 | T-05-01/02 | PRAGMA `case_sensitive_like=ON` sem regressão de busca/filtro/agrupamento | integration + suíte completa | `.venv/bin/python -m pytest tests/test_indices.py tests/test_database.py tests/test_repository.py -q` (task-level, ~3s); `scripts/verificar.sh --rapido` (86.2s medidos) movido para o portão de fim de wave | ✅ | ⬜ pending |
...
- [x] Feedback latency < 90s — verificado por cronometragem, não por estimativa (2026-08-17: task-level 2.7s; portão de wave 86.2s)
